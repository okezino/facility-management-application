...
class ApartmentRemoteMediator(
    private var feedId: String,
    private val apiService: ApiService,
    private val centralDatabase: CentralDatabase
) : RemoteMediator<Int, ApartComplaints>() {

    private var key: Long? = null

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, ApartComplaints>,
    ): MediatorResult {
        ...
        try {
            ...
            val apartComplaints = complains?.map { complain ->
                ApartComplaints(
            ...
            centralDatabase.withTransaction {
                ...
                if (loadType == LoadType.REFRESH) {
                    centralDatabase.apartRemoteKeysDao.clearRemoteKeys()
                    centralDatabase.apartComplainsDao.clearApartComplains()
                }
                val prevKey = if (page == NETWORK_STARTING_PAGE) null else page - 1
                val nextKey = if (endOfPaginationReached) null else page + 1
                val keys = complains?.map {
                    ApartmentRemoteKeys(id = key, prevKey = prevKey, nextKey = nextKey)
                }
                centralDatabase.apartRemoteKeysDao.insertAll(keys)
                centralDatabase.apartComplainsDao.saveApartComplaints(apartComplaints)
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ApartComplaints>): ApartmentRemoteKeys? {
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ApartComplaints>): ApartmentRemoteKeys? {
