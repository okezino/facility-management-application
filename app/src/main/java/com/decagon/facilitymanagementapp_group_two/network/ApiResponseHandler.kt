...
data class ApiResponseHandler<T>(
    val resultStatus: ResultStatus<T>,
    var action: ((result: ResultStatus<T>) -> Unit)? = null,
    var view: View? = null
) {
    ...
    private fun handleNetworkResponse() {
        ...
        when (resultStatus) {
            ...
            is ResultStatus.GenericError -> {
                Log.d("ApiCall Error", "${resultStatus.code}")
                message = when (resultStatus.code) {
            ...
            is ResultStatus.Success -> action?.invoke(resultStatus)
        }
    }
}
